...
class SPRBranchedPython{
public:
    SPRBranchedPython(const std::string ini_path){
    ...
    SPRBranchedPython(const std::map<std::string, float> init_params)
    {
        Y =  pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>);
    ...
    Eigen::MatrixX3f get_joint_location() const{
        ...
        bool load_branch_array_from_np(std::vector<std::vector<int>> data){
        std::vector<Eigen::Index> branchIndices;
        std::vector<int> dataVec;
        for (std::size_t i=0; i<data.size(); ++i){
            dataVec = data[i];
            branchIndices.clear();
            for(std::size_t j=0; j<dataVec.size(); ++j){
                branchIndices.push_back((Eigen::Index)dataVec[j]);
            }
            B.push_back(branchIndices);
        }
        brancharray_flag = true;
        return true;
    }

    std::vector<std::vector<int>> get_branch_array() const{
        ...
        for (std::size_t i=0; i<B.size(); ++i){
            ...
            for(std::size_t j=0; j<branchIndices.size(); ++j){
                dataVec.push_back((int) branchIndices[j]);
            }
    ...
    bool compute_spr(){
        ...
        X_registered = spr_algorithm->SPR::computeEM(X, Y);
        return true;
    };

    bool compute_sprbranched(){
        if (!param_flag){
            py::print("The initial parameters for SPR algorithm have not been correctly loaded, please load them first");
            return false;
        }
        ...
        if (!brancharray_flag){
            py::print("No branch array was provided, please load it");
            return false;
        }
        X_registered = spr_algorithm->computeEM(X, Y, B);
        return true;
    };
